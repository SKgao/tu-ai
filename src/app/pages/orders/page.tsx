...
import type { OrderFilterValues, OrderListResult, OrderQuery, OrderRecord } from './types';

const INITIAL_FILTERS: OrderFilterValues = {
  tutuNumber: '',
  orderNo: '',
  itemId: undefined,
  payType: undefined,
...
function normalizeSearchValues(values: OrderFilterValues): Partial<OrderQuery> {
  return {
...
export function OrderManagementPage() {
  ...
  const {
    ...
    setPageSize,
    reload,
  } = useRemoteTable<OrderQuery, OrderListResult, OrderRecord>({
    initialQuery: INITIAL_QUERY,
    request: async (currentQuery) => (await listOrders(currentQuery)) as OrderListResult,
  ...
  useMountEffect(() => {
    return ensureOrderFilterOptions().catch((error) => {
      message.error(getErrorMessage(error, '筛选项加载失败'));
    });
  });

  const handleSearch: FormProps<OrderFilterValues>['onFinish'] = (values) => {
  ...
  return (
    <div className="page-stack">
      ...
      <Card title="订单列表" extra={<Typography.Text type="secondary">共 {totalCount} 条记录</Typography.Text>}>
        <Table<OrderRecord>
          rowKey={(row) => String(row.orderNo || row.id || 'order')}
          columns={columns}
          dataSource={orders}
          loading={loading}
          scroll={{ x: 1480 }}
