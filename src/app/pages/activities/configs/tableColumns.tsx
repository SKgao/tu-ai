...
type CreateActivityColumnsOptions = {
  onEdit: (activity: ActivityRecord) => void;
  onToggleStatus: (activity: ActivityRecord, status: number) => void | Promise<void>;
  onDelete: (activity: ActivityRecord) => void | Promise<void>;
  submitting: boolean;
};
...
export function createActivityColumns({
  ...
  onDelete,
  submitting,
}: CreateActivityColumnsOptions): TableColumnsType<ActivityRecord> {
  return [
    { title: '活动名称', dataIndex: 'title', render: (value) => String(value || '-') },
    {
      title: '活动内容',
      dataIndex: 'content',
    ...
    },
    {
      ...
      render: (value) => formatCurrencyCent(value as string | number | null | undefined),
    },
    {
      title: '活动状态',
      dataIndex: 'status',
    ...
    { title: '活动持续时间', dataIndex: 'activeExpireDays', render: (value) => String(value ?? '-') },
    { title: '活动开始时间', dataIndex: 'beginAt', render: (value) => String(value || '-') },
    { title: '活动结束时间', dataIndex: 'endAt', render: (value) => String(value || '-') },
